...
Avoid keeping a single conversation alive across unrelated work.
...
**Better**
- One session = one logical task
- Use `/clear` or start a fresh session when switching contexts

Why: Claude repeatedly reprocesses conversation history and may trigger expensive context compaction.

---

### 2. Keep `CLAUDE.md` Minimal
Everything in `CLAUDE.md` is loaded into every session.
...
Move infrequently used information into:
- `.claude/skills/`
- Separate documentation files

Why: Reducing always-loaded context lowers token consumption across every interaction.

---

### 3. Avoid Large Terminal Outputs
Large logs are often the biggest hidden source of token usage.
...
```bash
npm test 2>&1 | grep FAIL -A 20
```

Or:
- Run only affected tests
- Filter logs before showing them to Claude
- Inspect failures manually when possible

Why: A large test log can consume more tokens than the actual coding task.

---

### 4. Use Lower Effort Levels for Routine Work
Not every task requires deep reasoning.
...
Reserve higher effort settings for:
- Architecture decisions
- Complex debugging
- Research-heavy tasks

Why: Higher reasoning effort increases token usage significantly.
...
Compact before the context window becomes large:
...
- Compact around 60–70% context usage
- Don't wait for automatic compaction

Why: Summarizing a smaller history is much cheaper than summarizing a nearly full context window.

### Highest-ROI Habits

If you only adopt three changes:

1. Start a fresh session for each task
2. Keep `CLAUDE.md` extremely small
